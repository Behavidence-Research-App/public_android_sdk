...
class AppSessionIOImpl extends ClientParent implements AppSessionsIO {
    ...
    public void deletePendingSessions(long beforeTime) {
        database.appSessionDao().deletePendingSessions(beforeTime);
    }

    @NonNull
    @Override
    public List<AppSession> getAppSessions(long lastTimeUpload) {
        List<AppSession> appSessions = database.appSessionDao().getSessions(lastTimeUpload);

        if (appSessions == null)
            appSessions = new ArrayList<>();
    ...
    public void refreshAppSessions() {
        if (!isAccessPermissionEnabled(context))
            Log.e("BehavidenceError", "Access Permission Denied. Please grant access permission");
        else EventsLog.refreshAppSessions(this);
    }
    ...
    private void putAppSessions(@NonNull List<AppSession> appSessions) {
        database.appSessionDao().insert(appSessions);
    }
    ...
    private List<AppSession> getIncompleteSessions() {
        return database.appSessionDao().getPendingSessions();
    }
